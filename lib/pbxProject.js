...
pbxProject.prototype.allUuids = function(log = (message) => {}) {
    log(`allUuids.start`);
    var sections = this.hash.project.objects,
        uuids = [],
        section;
    log(`allUuids.beforeLoop`);
    for (key in sections) {
        log(`allUuids.key=${key}`);
        section = sections[key]
        uuids = uuids.concat(Object.keys(section))
    }
    log(`allUuids.afterLoop`);
    uuids = uuids.filter(function(str) {
        return !COMMENT_KEY.test(str) && str.length == 24;
    });
    log(`allUuids.uuids=${uuids.length}`);
    return uuids;
}

pbxProject.prototype.generateUuid = function(log = (message) => {}) {
    log(`generateUuid.start`);
    var id = uuid.v4()
        .replace(/-/g, '')
        .substr(0, 24)
        .toUpperCase()
    log(`generateUuid.start=${id}`);
    if (this.allUuids(log).indexOf(id) >= 0) {
        log(`generateUuid.collision`);
        return this.generateUuid();
    } else {
        log(`generateUuid.returning=${id}`);
        return id;
    }
}
...
pbxProject.prototype.addBuildPhase = function(filePathsArray, buildPhaseType, comment, target, optionsOrFolderType, subfolderPath, log = (message) => {}) {
    log('addBuildPhase.start');
    var buildPhaseSection,
        fileReferenceSection = this.pbxFileReferenceSection();
    var buildFileSection = this.pbxBuildFileSection();
    log(`addBuildPhase.pbxBuildFileSection`);
    var buildPhaseUuid = this.generateUuid(log);
    log('addBuildPhase.generatedUuid');
    var buildPhaseTargetUuid = target || this.getFirstTarget().uuid;
    log(`addBuildPhase.getFirstTarget=${buildPhaseTargetUuid}`);
    var commentKey = f("%s_comment", buildPhaseUuid);
    log(`addBuildPhase.commentKey=${commentKey}`);
    var buildPhase = {
            isa: buildPhaseType,
    ...
    log('addBuildPhase.buildPhase');
    var filePathToBuildFile = {};
    log('addBuildPhase.filePathToBuildFile');
    ...
    if (buildPhaseType === 'PBXCopyFilesBuildPhase') {
        ...
        log('addBuildPhase.PBXCopyFilesBuildPhase');
        ...
        log('addBuildPhase.PBXShellScriptBuildPhase');
    }

    if (!this.hash.project.objects[buildPhaseType]) {
        log(`addBuildPhase.!this.hash.project.objects[buildPhaseType]`);
        this.hash.project.objects[buildPhaseType] = new Object();
    }

    if (!this.hash.project.objects[buildPhaseType][buildPhaseUuid]) {
        log(`addBuildPhase.!this.hash.project.objects[buildPhaseType][buildPhaseUuid]`);
    ...
    if (this.hash.project.objects['PBXNativeTarget'][buildPhaseTargetUuid]['buildPhases']) {
        log(`addBuildPhase.this.hash.project.objects['PBXNativeTarget'][buildPhaseTargetUuid]['buildPhases']`);
    ...
    for (var key in buildFileSection) {
        log(`addBuildPhase.key=${key}`);
        // only look for comments
        if (!COMMENT_KEY.test(key)) continue;

        var buildFileKey = key.split(COMMENT_KEY)[0];
        log(`addBuildPhase.buildFileKey=${buildFileKey}`);
        log(`addBuildPhase.project.objects=${this.hash.project.objects}`);
        log(`addBuildPhase.buildFileSection=${buildFileSection}`);
        var buildFile = buildFileSection[buildFileKey];
        log(`addBuildPhase.buildFile=${buildFile}`);
        fileReference = fileReferenceSection[buildFile.fileRef];

        log(`addBuildPhase.fileReference=${fileReference}`);

        if (!fileReference) continue;

        log(`addBuildPhase.fileReference.notNull`);

        var pbxFileObj = new pbxFile(fileReference.path);

        log(`addBuildPhase.pbxFileObj`);
    ...
    for (var index = 0; index < filePathsArray.length; index++) {
        ...
        log(`addBuildPhase.filePath=${filePath}`);

...
pbxProject.prototype.generateUuid = function() {
    log('Generating UUID, existing count:', this.allUuids().length);
    var id = uuid.v4()
        .replace(/-/g, '')
        .substr(0, 24)
        .toUpperCase();
    ...
    log('Generated:', id);
    log('Exists:', this.allUuids().indexOf(id) >= 0);

    if (this.allUuids().indexOf(id) >= 0) {
        log('UUID collision, regenerating...');
...
pbxProject.prototype.addBuildPhase = function(filePathsArray, buildPhaseType, comment, target, optionsOrFolderType, subfolderPath, logger = (message) => {}) {
    log = logger;
